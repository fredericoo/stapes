...
- **A floor with something standing on it** — used to be. A bare floor
  hard-sealed the vertical flood edge; the same floor with a bush, a sign or a
  fence on it was half opaque, which disqualified it, and daylight came through
  the ground at half strength. That was a bug rather than a rule — the seal is
  a property of the floor and the opacity a property of the thing standing on
  it — and `app/lib/lighting.ts` now decides a vertical edge on the seal alone.
  It is kept here because the generator's `SEALED_ROOF` rule was written
  against the bug and still refuses to run a cave under a cell with a prop on
  it, which is now stricter than it needs to be.
