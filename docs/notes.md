...
**Most of it is one line in `scopedPatchFor`.** A hidden body other than the
viewer's own is kept out of `held`, and `held` already decides the cells (a
placement whose owner is not held is stripped), the actor-scoped events and
every per-body diff. `actorsInReach` does the same for `hello`. The toggle then
