...
- `bun run generate:trees` — render every tree species, `tree` included, from
  small 3D models in the game's own projection, quantised to the palette. A
  species with several models becomes a scatter tile with one face per model.
  Grows `data/tilesets/tiny-ranch-tiles.png` from row 20 down as needed and
  keeps `data/tilesets.json` in step.
