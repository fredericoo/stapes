...
export default defineConfig({
  test: {
    ...
    // in package.json: pure logic runs in the fast generic runner, while the
    // world runs in the runtime it deploys to, against a real database. Vitest
    // cannot be that runner — it drives tests through worker threads, and the
    // database is a native module that does not survive the trip.
    include: ["app/**/*.test.ts"],
