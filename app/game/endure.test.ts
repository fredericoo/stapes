...
function world(cells: { at: Coord; stack: string[] }[]): MapFile {
  ...
  return replaceStack(map, 9, 9, 0, [{ tileId: "dirt" }, { tileId: "player", direction: "s" }]);
}
...
describe("EndureIndex", () => {
  ...
  it("refuses a status the tile does not suffer", () => {
    ...
    expect(endure.afflict(ORIGIN, "grass", grassEndure(), chill)).toBe(false);
  ...
  it("spends the pool at the status's own cadence", () => {
    ...
    expect([...endure.afflicted()][0]?.hp).toBe(GRASS_DURABILITY - BURN_PER_SECOND * 2);
  });
  ...
  it("carries the caster and the elements onto what it hands back", () => {
    const endure = index();
    endure.afflict(ORIGIN, "grass", grassEndure(), BURNED, undefined, "arcanist", ["fire"]);
...
describe("what is reported as alight", () => {
  ...
  it("reaches the snapshot, and goes quiet when the fire does", () => {
    const play = new GameSession(world([{ at: ORIGIN, stack: ["grass", "flame"] }]), tiles, {
      statuses: catalogue,
    });
    play.tick(TICK_MS);
...
describe("spreadShares", () => {
  ...
  it("lets the last embers go out rather than laying an infinitely thin burn", () => {
    ...
    expect(spreadShares(map, { ...consumed, remainingMs: 3 }, tilesById)).toEqual([]);
  });
...
describe("the content in data/", () => {
  ...
  it("takes a tree down, given a flame that does not go out", () => {
    const play = burn([{ at: ORIGIN, stack: ["dirt", "tree", "flame"] }], 60_000);
    expect(stackIds(play.getMap(), 0, 0)).toEqual(["dirt", "flame"]);
  });
